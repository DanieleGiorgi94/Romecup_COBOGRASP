void disegno(){
  ...
  wrist = asin(as.getAccelerometerSensorValues()[0]/g);
  lean = -asin(as.getAccelerometerSensorValues()[1]/(g*cos(wrist)));
  plan = -radians(as.getOrientationSensorValues()[0]);
  //180 : pi = x : gradi ---> x = 180 * gradi / PI
